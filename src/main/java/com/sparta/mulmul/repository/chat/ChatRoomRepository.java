...
public interface ChatRoomRepository extends JpaRepository<ChatRoom, Long> {
    ...
    @Query(value =
            "SELECT DISTINCT r.room_id AS roomId, r.acc_out AS accOut, r.req_out AS reqOut, r.acc_fixed AS accFixed, r.req_fixed AS reqFixed, " +
                    ...
                    "msg.message AS message, msg.created_at AS date, COUNT(IF(ban.is_banned = true, ban.is_banned, null)) AS isBanned " +
            "FROM chat_room r " +
                    ...
                    "LEFT JOIN chat_banned ban ON ( r.acceptor_id = ban.user_id AND r.requester_id = ban.banned_user_id ) " +
                        "OR ( r.requester_id = ban.user_id AND r.acceptor_id = ban.banned_user_id ) " +
